...
async def get_statistics():
    ...
    # Overall statistics (compute averages per-day)
    # Aggregate units_sold by date first to get true daily totals
    daily_sales = df.groupby(df['date'].dt.strftime('%Y-%m-%d'))['units_sold'].sum()
    stats = {
        "total_rows": len(df),
        ...
        "sales_summary": {
            ...
            "avg_daily_units": float(daily_sales.mean()) if len(daily_sales) > 0 else 0.0,
            "max_daily_units": float(daily_sales.max()) if len(daily_sales) > 0 else 0.0
        }
    }
...
async def get_model_metrics():
    """Return basic model metrics: number of trained models, avg MAPE and avg RMSE.

    Checks in-memory `model_store` first, then falls back to saved metadata in `saved_models/metadata.json`.
    """
    # Check in-memory forecaster
    if 'forecaster' in model_store:
        forecaster = model_store['forecaster']
        models_trained = len(getattr(forecaster, 'models', {}))
        metrics_df = getattr(forecaster, 'metrics', None)
        if metrics_df is not None and hasattr(metrics_df, 'empty') and not metrics_df.empty:
            try:
                avg_mape = float(metrics_df['val_mape'].mean())
            except Exception:
                avg_mape = 0.0
            try:
                avg_rmse = float(metrics_df['val_rmse'].mean())
            except Exception:
                avg_rmse = 0.0
        else:
            avg_mape = 0.0
    ...
    # Fallback: read saved metadata if available
    ...
    if os.path.exists(metadata_path):
        try:
            with open(metadata_path, 'r') as f:
                meta = json.load(f)
            return {
                'models_trained': int(meta.get('n_products', 0)),
                'avg_val_mape': float(meta.get('avg_val_mape', 0.0)),
                'avg_val_rmse': float(meta.get('avg_val_rmse', 0.0))
            }
        except Exception:
            pass
...
async def optimize_inventory(request: InventoryRequest):
    ...
    """
    # Try to use existing forecast; if missing, attempt to generate a simple forecast
    forecast_data = forecast_store.get(request.product_id)

    if forecast_data is None:
        # If no dataset or no in-memory models, instruct user to run /forecast
        if 'current' not in dataset_store:
            raise HTTPException(status_code=404, detail="No dataset loaded. Please upload and run /forecast first.")

        if 'forecaster' not in model_store:
            raise HTTPException(status_code=404, detail="Forecast not found and no trained models available. Please run /forecast first.")

        try:
            # Build features and get last-known features for the product
            df = dataset_store['current'].copy()
            fe = FeatureEngineer()
            df_featured = fe.engineer_all_features(df)
            feature_cols = fe.get_feature_columns(df_featured)

            product_df = df_featured[df_featured['product_id'] == request.product_id].sort_values('date').tail(1)
            if len(product_df) == 0:
                raise HTTPException(status_code=404, detail="Product data not found in dataset.")

            last_features = product_df[feature_cols]

            forecaster = model_store['forecaster']
            model = forecaster.models.get(request.product_id)
            if model is None:
                raise HTTPException(status_code=404, detail="No trained model for this product. Run /forecast with retrain=true.")

            # Predict one step and repeat for a 30-day horizon (simplified)
            forecast_days = 30
            pred = model.predict(last_features, num_iteration=getattr(model, 'best_iteration', None))
            base = float(pred[0]) if hasattr(pred, '__len__') else float(pred)
            forecast_values = np.array([max(0, base) for _ in range(forecast_days)])
            forecast_std = float(np.std(forecast_values)) if len(forecast_values) > 1 else 1.0

            # Minimal forecast record
            forecast_data = {
                'product_id': request.product_id,
                'forecast_values': forecast_values.tolist(),
                'forecast_dates': [(pd.to_datetime(product_df['date'].iloc[0]) + timedelta(days=i+1)).strftime('%Y-%m-%d') for i in range(forecast_days)],
                'lower_bound': (forecast_values - 1.96 * forecast_std).tolist(),
                'upper_bound': (forecast_values + 1.96 * forecast_std).tolist(),
                'forecast_std': forecast_std
            }

            forecast_store[request.product_id] = forecast_data

        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Failed to generate forecast for product: {str(e)}")

    try:
        # Run optimization using forecast_data
        forecast_values = np.array(forecast_data['forecast_values'])
        optimizer = InventoryOptimizer(service_level=0.95)
        forecast_std = float(forecast_data.get('forecast_std', np.std(forecast_values) if len(forecast_values) > 0 else 1.0))
...
async def get_product_data(product_id: str):
    ...
    # Basic statistics (compute averages per-day)
    daily_sales = product_df.groupby(product_df['date'].dt.strftime('%Y-%m-%d'))['units_sold'].sum()
    stats = {
        "product_id": product_id,
        ...
        "sales": {
            ...
            "avg_daily_units": float(daily_sales.mean()) if len(daily_sales) > 0 else 0.0,
            "max_daily_units": float(daily_sales.max()) if len(daily_sales) > 0 else 0.0,
            "min_daily_units": float(daily_sales.min()) if len(daily_sales) > 0 else 0.0
        }
    }
...
async def list_products(limit: int = Query(100, ge=1, le=1000)):
    ...
    # Get product summary with per-day averages
    # First aggregate per product per date to compute daily totals, then summarize per product
    df['date'] = pd.to_datetime(df['date'])
    daily = df.groupby(['product_id', df['date'].dt.strftime('%Y-%m-%d')])['units_sold'].sum().reset_index()
    daily.columns = ['product_id', 'date', 'daily_units']

    product_summary = daily.groupby('product_id').agg(
        total_sales=('daily_units', 'sum'),
        avg_daily_sales=('daily_units', 'mean'),
        first_date=('date', 'min'),
        last_date=('date', 'max')
    ).reset_index()
    ...
    for p in products:
        ...
        p['avg_daily_sales'] = float(p['avg_daily_sales']) if p['avg_daily_sales'] is not None else 0.0
